package action {
	...
	public class ScaleBox extends UIComponent {
		...
		private function drawline(w:Number, h:Number):void
		{
			...
			rect.height=w + 2;
			rect.width=w + 2;
			//drawRect(0xFFFFFF, rect);//modified by james
			rect.left=0;
			rect.top=0;
			rect.height=w;
			rect.width=h;
			//drawRect(0, rect);//modified by james
			//hitSprite.graphics.lineStyle(1, 0xFFFFFF, 0.5);
			hitSprite.graphics.lineStyle(5, 0xFFFFFF, 0.5);//modified by james
			hitSprite.graphics.beginFill(0, 0);
			//hitSprite.graphics.drawRect(1.0, 1.0, w - 2, h - 2);
			hitSprite.graphics.drawRect(1.0, 1.0, w-2, h-2);//modified by james
